/* =========================================================
   v3-script.js — 江沁钊个人主页交互
   V3：新增问题反馈表单（校验 + 提交到 Supabase），其余交互沿用 v2
   ========================================================= */
...
/* =========================================================
   V3 新增：问题反馈表单 —— 提交到 Supabase（public.user_feedback）
   ---------------------------------------------------------
   目标表 public.user_feedback 的列：
     id（主键，数据库生成）、name、device、content、created_at（默认 now()）

   安全说明：这里用的是 Supabase 的 publishable key（等同于旧版的 anon key），
   它被设计为可以公开出现在前端代码里；真正决定"谁能写"的是数据库的 RLS 策略。
   ========================================================= */
(function () {
  "use strict";

  var SUPABASE_URL = "https://joqhbooccydgajunwnwf.supabase.co";
  var SUPABASE_KEY = "sb_publishable_bMqCHijRP26jLk4woTAlnw__qgwra18";
  var TABLE = "user_feedback";
  var DRAFT_KEY = "v3-feedback-draft";
  var MAX_LEN = 1000;

  var form = document.getElementById("fbForm");
  if (!form) return; /* 页面上没有这张表单（例如旧版页面）就直接跳过 */
  ...
  var nameErrEl = document.getElementById("fbNameErr");
  var contentErrEl = document.getElementById("fbContentErr");
  ...
  function setStatus(msg, kind) {
    ...
    statusEl.className = "fb__status" + (kind ? " is-" + kind : "");
  }

  function lock(on) {
    if (!submitEl) return;
    submitEl.disabled = !!on;
    submitEl.textContent = on ? "提交中…" : "提交反馈";
  }

  /* ---------- 设备默认值：按 UA 猜一个，访客随时可以改 ---------- */
  function guessDevice() {
    var ua = navigator.userAgent || "";
    var touchPoints = navigator.maxTouchPoints || 0;
    /* iPadOS 13+ 的 UA 伪装成 Macintosh，只能靠触点数把它认出来 */
    if (/iPad|Tablet|PlayBook|Silk/i.test(ua) || (/Macintosh/i.test(ua) && touchPoints > 1)) {
      return "平板";
    }
    if (/Android|iPhone|iPod|Mobile|Windows Phone/i.test(ua)) {
      return "手机";
    }
  ...
  function applyDeviceGuess() {
    if (!deviceEl) return;
    var guess = guessDevice();
    for (var i = 0; i < deviceEl.options.length; i++) {
      if (deviceEl.options[i].value === guess) {
        deviceEl.selectedIndex = i;
        return;
      }
    }
  }
  ...
  function loadDraft() {
    ...
    if (d.device && deviceEl) deviceEl.value = d.device;
    if (d.content && contentEl) contentEl.value = d.content;
  ...
  function validate() {
    ...
    if (!name) {
      setErr(nameErrEl, "请留下你的名字，我好知道这是谁的建议。");
      ok = false;
    }
    if (content.length < 5) {
      setErr(contentErrEl, "再多写几个字吧（至少 5 个字），这样我才知道问题出在哪。");
      ok = false;
    }
    return ok;
  }

  /* ---------- 提交到 Supabase（走它的 PostgREST 接口） ---------- */
  function postFeedback(payload) {
    return window.fetch(SUPABASE_URL + "/rest/v1/" + TABLE, {
      method: "POST",
      headers: {
        apikey: SUPABASE_KEY,
  ...
  if (deviceEl) {
    deviceEl.addEventListener("change", saveDraft);
  }
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (!validate()) {
      if (nameErrEl && nameErrEl.textContent && nameEl) {
        nameEl.focus();
      } else if (contentErrEl && contentErrEl.textContent && contentEl) {
        contentEl.focus();
      }
      return;
    }
    ...
    postFeedback(payload)
      ...
      .then(function (r) {
        if (r.ok) {
          ...
          clearDraft();
          refreshCount();
          applyDeviceGuess(); /* reset 会把下拉框推回第一项，这里恢复本次访客的默认值 */
          setStatus("收到啦，谢谢你花时间写这些。", "ok");
        } else {
          var code = parseError(r.body);
          setStatus(
            "没能提交成功（" + (code || "HTTP " + r.status) + "）。如果一直失败，也欢迎直接发邮件给我。",
            "err"
          );
        }
      })
      .catch(function () {
        setStatus("网络好像不太顺，没能发出去。请稍后再试一次。", "err");
      })
      .then(function () {
        lock(false);
      });
