...
| 项目 | 初始 | 第一轮（调亮一些） | 第二轮（再亮一点 · 最终） |
| :-- | :-- | :-- | :-- |
| `.hero__video` filter | `brightness(.62) saturate(.92) contrast(1.02)` | `brightness(.80) saturate(.96) contrast(1.04)` | **`brightness(.90) saturate(.98) contrast(1.06)`** |
| `.hero__veil` 放射渐变 | 中心 α.30 / 边缘 α.66 | 中心 α.18 / 边缘 α.54 | 中心 **α.14** / 边缘 **α.46** |
| `.hero__veil` 纵向渐变 | .62 / .42 / .52 → #08131f | .44 / .24 / .34 → #08131f | **.28 / .14 / .22** → #08131f |
| `.hero__veil` 窄屏（≤720px） | 同桌面 | 同桌面 | **单独加强**：放射 .22/.54、纵向 .36/.22/.30 |

**遮罩底部始终是 100% `#08131f`** —— 这是"页 1 → 页 2 无接缝"的唯一依据，两轮调亮**都没动它**。

像素统计复核（`System.Drawing` 采样，网格固定：桌面 hero = (0,60)–(1440,500)，移动 hero = (0,60)–(390,420)）：

| 指标 | 初始 | 第一轮 | 第二轮（最终） |
| :-- | --: | --: | --: |
| 桌面 hero 平均亮度 | 31.2/255 | 48.7/255 | **63.8/255** |
| 移动 hero 平均亮度 | 45.9/255 | 62.0/255 | **66.8/255** |
| 桌面 hero 纹理 stdev | 40.2 | 37.8 | 36.0 |
| 桌面纯渐变区 stdev（对照） | 6.7 | 7.2 | 7.2 |
| 桌面截图体积 | 498,468 B | 599,191 B | 624,899 B |

**为什么要给窄屏单独加强**：第二轮改完桌面 63.8/255 在安全区内，但移动端冲到 **75.8/255 —— 已越过对比度阈值**。
加 `@media (max-width:720px)` 单独把移动端遮罩调强后回到 **66.8/255**；桌面实测仍 63.8，**完全不受影响**。
教训：**调亮必须桌面/移动分别量化**，移动端文字更大更满、且常压在高亮天空上，是最先出问题的一侧。

文字安全垫：`.hero__title` / `.hero__lead` 本身带 `text-shadow: 0 8px 40px rgba(0,0,0,.25)` / `0 2px 20px rgba(0,0,0,.35)`，
所以亮度阈值不是硬上限；这两轮都留有充足余量。

> stdev 略降不是"变糊"：调暗时大量像素被压到 0 造成**截断堆积**，反而把方差顶高了；
> 提亮后直方图铺开，方差回落。hero 区 36.0 仍**远高于**渐变区的 7.2 → 证明渲染的确实是实拍画面。
