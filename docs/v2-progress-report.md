...
## v2 · 追加迭代（第十次：把"横向填充"改成"一笔一笔写出"）

### 🎯 现象（用户反馈原话）
> "颜色对了，但是需要的是之前那种一笔连贯的书写出来的感觉，不要这种从左向右填充的感觉。"

即：第九次虽然解决了棱角与配色，但**写出方式**是错的——一条圆头粗描边**从左向右横扫**做遮罩推进，
观感就是"横向擦除/填充"，而不是"笔尖顺着笔画一笔一笔走"。

### ⚙️ 新方案（沿笔画测地距离场，字是被"走"出来的）
放弃 SVG 遮罩，改 **`<canvas>` 逐像素渲染**。`v2-index.html` 里 `#aboutScreenWord` 内换成
`<canvas class="word-canvas" id="wordCanvas">` + `<span class="word-pen" id="wordPen">`（内嵌纸飞机路径）。

`v2-script.js` 的 about IIFE 整块重写：

1. **量字取墨**：Pacifico 居中 `fillText` 到离屏 canvas → `getImageData` → `alpha > 8` 得墨迹蒙版
   （不依赖字体指标，字随字体子集变化也不会错位）。
2. **拆笔画块**：8 邻接 flood fill 找出连通块（"about me" 实测 2 块：`about` / `me`）；
   每块取**最左列最上像素**为起笔点。
3. **块内测地距离场**：块内 Dijkstra（8 邻接，直边 1 / 斜边 1.414，自写二叉堆 + 惰性删除）。
   距离 ≈ **笔尖沿笔画走过的路程**，所以同一条竖笔的上下端时间也不同——这是"顺笔画写"的关键。
4. **块间串行分配时间**（本轮修掉的核心缺陷）：按起笔点 x 从左到右排先后，
   **书写窗口互不重叠**，每块时长 ∝ 该块路径长度（掺一半"均分"，免得字母上的点来不及看清）。
   → **任何时刻只有一处笔尖**。
5. **逐帧绘制**：`t ≤ p` 写彩虹（按**列**取色，左绿右蓝铺满整词），`t > p` 写 12% 白幽灵；
   同一帧取"刚写过的像素质心"喂给纸飞机 → **笔尖始终贴在书写前沿**。
6. 其余沿用：`smoothstep` + 2000ms、单词顶边越过视口 60% 触发、`?shot=1` / reduced-motion 直接定格。

### 🔍 为什么第一版还不够（同轮内自查发现的 bug）
初版给各块按 `SKEW=0.42`（按起笔点 x 归一化）**错开起笔**、每块时长 `SPAN=0.58`
→ 两块**时间窗重叠**，`about` 还没写完 `me` 就开笔，
**笔尖质心跳到 "m" 附近**（实拍：粉色笔点在 about 与 me 之间，而着色只到 "abo"）。
改成第 4 步的**串行窗口**后，笔尖中心 **x 695.5**、书写前沿 **745** ✓（紧贴前沿）。

### ✅ 验证（数字证据，非目测）
`artifacts` 之外的取证脚本 `.deepworks/tmp/wordstat.ps1`：以**定格帧**的饱和像素为"字形蒙版"，
逐列比对中间帧的着色覆盖；先做 **±4px 对齐搜索**排除"两帧错位"的假阳性。

| 项目 | 结果 | 说明 |
| :-- | :-- | :-- |
| 字形墨迹（蒙版） | **44,750 px** | 定格帧 |
| 已写比例 | **27,387 / 44,750 = 61.2%** | 中间帧 |
| 最佳对齐 dx | **0**（27,387，明显压过 ±1 的 26,593） | 两帧同位置，无位移伪影 |
| ±1px 容差后 | 27,490（**几乎不变**） | 部分着色不是边缘对不齐造成的 |
| **部分揭示的列** | **196 / 537 触及列 = 36.5%** | **横向擦除只会是 ~0%**（决定性证据） |
| 最右着色列 | 745（词宽 373..1072，约 53%） | 只写到中段 |
| 笔尖位置 | stray 饱和像素 **34 px**，bbox x[668..723]，中心 **695.5** | 紧贴前沿左侧 ✓ |

视觉描述交叉验证：中间帧 **"abou" 已着色、"t" 只填了一半、"me" 仍灰暗**；
定格帧整词彩虹完整（绿→黄→橙→红→粉→紫→蓝）；移动端同样"abo 已着色 / ut me 灰暗" ✓。

### 🧰 本轮踩到的坑（PowerShell 取证脚本）
1. **PowerShell 变量名大小写不敏感**：脚本里把亮度写成 `$x1`，直接**覆盖了参数 `$X1`（右边界）**
   → 循环瞬间退出、蒙版统计为 **0**，看起来像"机制完全没生效"。（改用 `$hi1` / `$lo1`。）
2. 函数 `return @{...}` 在本机 5.1 下取属性拿到空值 → 改成**完全内联**读位图，问题消失。

### 📸 本轮截图（已换新，旧的横向遮罩版作废）
`artifacts/screenshots/`：`v2-about-word-writing.png`（书写中，笔尖在前沿）/
`v2-about-word-rest.png`（定格彩虹）/ `v2-about-word-mobile.png`（移动端）。

### ⚠️ 遗留
- 第九次记下的**待办未动**：about-screen 下沿（doc y=1800）与视差天幕交界仍有 **+11.6** 逐行亮度跳变，
  照接缝做法用同色渐隐窄带压到 <1.5（下一轮候选）。
- `v2-web/v2-about-data.js` 已不被引用，仍在磁盘（删除需用户确认）。

本轮改 HTML/CSS/JS → 版本号 `?v=10` → **`?v=12`**（两处；轮内先 v11、修复笔尖漂移后再升 v12）。

## v2 · 追加迭代（第十一次：笔顺回到数据本身 —— 手写单词改成"最近轨迹点弧长"揭示）

### 🎯 现象（用户反馈原话）
> "之前最开始还不是彩色的时候笔画都是对的，你直接按那个版本的笔画顺序不就完了"

同时用户明确要求：**不要再用截图分析来评审**，改成让页面自己回传数字（本轮验证方式因此整体换成"页面自报数据"）。

### 🔍 根因（第十次那套"沿笔画测地距离场"为什么必然错）
先证明"骨架→时间"的映射没错：`skT`（骨架像素的时间值）随 x 单调递增，说明映射链本身没问题。
病根在**"距离"的语义**：Dijkstra 累加的是**沿墨迹的像素路程**，而笔尖沿轨迹走的是**轨迹弧长**——
笔1（'a'+'b' 连笔）轨迹弧长 **892px** 却只横跨 **179px**（绕圈 + 升部），于是"早笔的长度优势"被放大成"时间优势"：
**空间上紧挨着、但笔序很晚**的墨会被它们最近的早期骨架点提前染色。
逐笔时间窗实测：第一笔吃掉了 **69% 的墨迹**（x 32..468，几乎整个 "about"）→ 观感就是**"一大片横向一起亮"**。
结论：只要用"加性像素距离"，就必然把"空间相邻"和"笔序靠前"混为一谈，**这个方向无解**。

### ⚙️ 新方案（笔顺 = 数据；算法只负责"贴合字形"）
`v2-about-data.js` 里本来就存着**真人书写顺序**的 9 条 SVG path（第九次迭代前就是按它写的，当时只是被拿去描摹字形才产生棱角）。
现在把它**当笔顺真源**复用，字形仍用平滑的 Pacifico 字体：

1. **③ 轨迹采样**：把 9 条 path 塞进隐藏 `<svg>`，`getTotalLength()` + `getPointAtLength()` 每 ~1.5px 取一点 →
   **x、y 分别线性映射**到墨迹 bbox（`ksx/ksy`），累积弧长 `trkS`，笔间加 `GAP = 词宽 × 0.012`；`trkPen[]` 标记收笔处。
   实测 `trkN = 2087` 点、`trkL = 3172`。
2. **④ 揭示时刻 = 最近轨迹点的弧长**：对每个墨迹像素，在笔尖轨迹上找**离它最近**的那一点，取其弧长。
   16px 网格分桶 + **逐圈外扩**（外圈不可能更近就停）+ 全扫兜底，避免 45,960 × 2087 的全量比较。
3. **⑤ 归一化**：`inkT = 最近点弧长 / trkL`；笔尖 `pen(p)` 用**同一个** `target = p × trkL` 沿轨迹走
   ⇒ **笔尖到哪儿，哪儿的墨正好显完**；提笔空隙没有墨迹对应 ⇒ 画面上就是自然的短暂停顿。
4. **删除**：② Zhang-Suen 细化、②b 剪毛刺、骨架最近点、全局 Dijkstra 时间场、二叉堆（`hD/hI/hN/hCap` + `hPush/hPop`）、
   `DX/DY/DW`、`degA/sk/skN/settled/inkD`，以及整套临时诊断块（`segA/segB/segX`、`?worddebug`）。
   `v2-script.js` 从 1060 行降到 **956 行**（净 -104 行）。

### ✅ 验证（页面自回传数字，全程不看图）
`?worddebug=1` 让页面把统计 POST 回本机 8126 端口（`.deepworks/tmp/recv.py` 落盘），实测输出：

```
ink=45960 W=760 H=198 trkN=2087 trkL=3172 strokes=9
bbox=32..738 , 8..189
xbT=0.03,0.03,0.08,0.18,0.21,0.33,0.36,0.42,0.47,0.53,0.62,0.65,-,0.75,0.81,0.87,0.92,0.95,0.97
xmono=17/17          ← 17 个 x 分箱的平均书写时刻严格递增
cnt=2476,2116,2612,2801,2249,2083,2204,2682,2290,2199,1398,2124,2929,2319,1540,1867,2184,2941,2466,2480
s0 t=0.000..0.281 tx=32..211 n=14337 xr=32..226      ← 第一笔只覆盖自己的 'a'+'b'（原为 xr=32..468）
s1 t=0.300..0.462 tx=210..347 n=7682  xr=197..361
s2 t=0.473..0.538 tx=339..406 n=2640  xr=325..422
s3 t=0.566..0.652 tx=384..436 n=5107  xr=372..466
s4 t=0.681..0.694 tx=389..430 n=1868  xr=405..468      ← 't' 横
s5 t=0.735..0.774 tx=531..562 n=2418  xr=522..574      ← 'm' 竖1
s6 t=0.785..0.834 tx=554..603 n=2543  xr=540..607
s7 t=0.845..0.914 tx=596..679 n=3738  xr=577..687
s8 t=0.925..1.000 tx=677..738 n=4193  xr=661..738      ← 'e'
```

| 判据 | 结果 | 说明 |
| :-- | :-- | :-- |
| 时间分箱是否单调推进 | **xmono = 17/17** | 左端的墨先出现、右端最后出现，无回退 |
| 每笔时间窗 vs 该笔 x 范围 | 9/9 吻合 | 第一笔窗 `xr=32..226`，**不再吞掉整词**（原 32..468） |
| 各时间档墨迹量 | 1398 ~ 2941 | 原来的 31662 巨桶消失（旧版第一笔 69% 墨迹） |
| 笔画数 | **strokes=9** | 与 `v2-about-data.js` 的 9 条 path 一致（此前文档写"11 条"是错的，已纠正） |

另有"页面真的跑起来了"的旁证：在真实页面（非沙盒）里于 canvas 上取像，已揭示墨迹 **44,985 px**、横跨 `x=32..738 / y=8..189`，即整词写完且无崩溃。

### 🧰 本轮踩到的坑（写给下一位）
1. **改动 `build()` 后必须先改/删诊断块**：重构删掉了 `skN/maxD`，但临时诊断块还在引用它们 →
   页面抛 `ReferenceError` → 后面的 `fetch` 根本没执行，看起来像"动画坏了"（截图字节数从 492,442 掉到 421,258 才露馅）。
2. **无头 Edge 抓不到 JS 运行期报错**：`--enable-logging=stderr` 和 `--enable-logging --log-level=0 --log-file=…` 都拿不到 `console` 报错
   （用故意抛错的阳性对照页验证，命中 **0** 次）。所以"页面是否报错"只能靠**页面自制信号**（画布像素统计 / POST 回传）判断。
3. `--dump-dom` 三种变体（`--headless` / `--headless=old` / 加 `--virtual-time-budget`）**stdout 全是 0 字节**；
   **不给 `--screenshot` 时页面 JS 完全不执行**；`--screenshot=NUL` 无效、`--screenshot=*.dat` 报 `Unsupported screenshot image file type`。
   → 想"不产出 PNG 又跑到页面 JS"做不到，只能**写到 scratch 路径 → 同命令删除**。
4. **虚拟时间会把网络请求饿死**：`--virtual-time-budget` 下页面侧 `fetch` 大多在 Edge 退出前没发完（多次 POST 只落 1 条）。
   稳妥做法是**页面内累积、末尾一次性回传**。
5. 截图文件是**异步落盘**的，`shot.ps1` 必须轮询等文件出现（否则偶发读到半截/不存在）。
6. 小坑：Edge 会把 `bytes written to file …png` 写到 **stderr**，管道里捕获后会连带触发上层对图片的处理 → 统一加 `2>$null`。

### ⚠️ 遗留
- **about-screen 下沿横向硬边（+11.6 逐行跳变，文档 y=1800）仍未修**，做法照首屏→第二页接缝（压同色渐隐窄带，目标 < 1.5）。
- `?replay=1` / `?dur=` 循环重播 + "点单词重播"**暂留**，是否作为正式交付由用户定。
- `artifacts/screenshots/` 里三张手写单词截图仍是第十次那版（"遮罩推进"）的帧，**需换新**。

本轮改 HTML/JS → 版本号 `?v=16` → **`?v=17`**。

---

## v2 · 追加迭代（第十二次：about-screen 下沿横向硬边修复 —— 清零第十一次遗留项）

### 🎯 现象
- 01「关于」板块下沿（`.about-screen__bg` 与视差天幕交界）横贯天空有一条可见的**横向亮度台阶**。
- 用户定的做法：照首屏 → 第二页接缝那一套修，目标逐行跳变 **< 1.5**。

### 🔍 根因（不是"缺一条过渡带"）
- 真因是 `.about-screen__bg` 渐变**末档 alpha 仍是 `0.32`** —— 渐变色在最后一行被"截断"，与下方天幕直接相接成为台阶。
- 实测逐行平均亮度跳变：桌面 **52.96**，移动 **3.61 / 3.62**。

### ⚙️ 修复
- `v2-web/v2-style.css` L486-500 `.about-screen__bg`：末档 alpha `0.32` → **`0`**。
- ⚠️ **该值必须保持 0**，不要"顺手调亮"（这条已同步进 `project-continuity.md` §7）。

### 🔎 判据校正（本轮确立，后续沿用）
- **硬边 = 单行 STEP**（相邻两行平均亮度出现孤立跳变）；**连续多行的陡坡 RAMP 不算缺陷**。
- 验收阈值：逐行平均亮度最大跳变 **< 1.5**（本机噪声参考 ~0.9）。
- 校正原因：早前把"渐变自身的陡坡"误判成硬边，会追着正常渐变白改。

### ✅ 验证结果
| 位置 | 修复前 | 修复后 |
| :--- | ---: | ---: |
| 桌面 · 该交界 | 52.96 | **1.35** |
| 桌面 · 另两处交界 | 9.23 / 7.33 | **0.76 / 0.73** |
| 移动 · 该交界 | 3.61 / 3.62 | **1.22** |
| 全页 6 处交界巡检 | — | **STEP 全 0** |

- 桌面 + 移动双视口复测，全部 PASS。
- 用户明确表示**不需要再做截图取证**，本轮以数值为准并直接存档。

### 🏷️ 存档
- commit **`e02a562`** — `v2: 修掉第二页(about-screen)下沿横向亮度台阶`（2 files changed, 8 insertions(+), 2 deletions(-)）。
