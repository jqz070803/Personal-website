...
- **改首屏视频**：换镜头/时长改 `v2-web/tools/build-hero-loop.ps1` 顶部 `$edl`/`$fade` 后重跑；
  ...
  当前调校值：`brightness(.90) saturate(.98) contrast(1.06)`，遮罩 α 中心 .14 / 边缘 .46、纵向 .28/.14/.22；
  **窄屏（≤720px）另有单独加强的遮罩**（移动端文字更大更满，实测不加会超过对比度阈值）。
  ⚠️ **`.hero__veil` 纵向渐变的 `100%` 必须是纯 `#08131f`**（与 `.bg-parallax__sky` 顶色相同）——这是页 1→页 2 无接缝的唯一依据，调亮时不要动它。
  ⚠️ hero 区平均亮度建议**不超过 ~75/255**，且必须**桌面/移动分别量**（桌面安全不等于移动安全）。
