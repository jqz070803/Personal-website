...
### 7.3 足迹主标题下加「可点击看大图」提示（用户反馈）

**诉求**（用户原话）：在「山川湖海足迹」主标题下面加一行小字，引导读者可以点击查看大图。

**实现**：

| 位置 | 改动 |
| :--- | :--- |
| `v3-index.html` | `.section__head` 内、`.section__lead` **之后**插入 `.section__hint`（内联放大镜 SVG + 一行文案）；版本 `?v=3 → ?v=4` |
| `v3-style.css` | 紧跟 `.section__lead` 之后新增 `.section__hint` 与 `.section__hint svg` 两条规则 |

- **文案**：「点一下照片，可以看大图」（口语化，与上面「大地是最好的老师，风是最好的同行者。」的调性一致）。
- **为什么做成胶囊而不是一行纯文字**：上一行导语本身就是 `--ink-faint` 小字，再加一行灰字只会糊在一起、没人会注意；
  胶囊用 `--line` 细描边 + `rgba(77,163,255,.06)` 极淡底色 + 放大镜图标（`--accent`，15px，`stroke: currentColor`），
  一眼能认出是"操作提示"而不是又一句抒情，同时尺寸克制（0.82rem、32px 高），不抢标题。
- **居中方式**：`display: flex` + `width: fit-content` + `margin: 16px auto 0` + `max-width: 100%`。
  最初写的是 `display: inline-flex` 靠父级 `text-align: center` 居中，量出来桌面 598 / 视口中心 704、移动 89 / 195，
  看着像"偏了 7px"——复核后确认**布局没坏，是测量口径问题**：容器居中于 `clientWidth`（已扣掉 15px 滚动条），
  而我拿 `innerWidth` 当中心去比；换成块级 + 自动外边距后语义更稳，不再依赖行内盒 + 空白字符的行为。
- **不动的东西**：区块底部原有的 `.journey-note`（"※ 以上均为我在旅途中的实拍，会随足迹持续更新。"）保留 ——
  标题下这条管「怎么用」，图片下那条管「是什么」，各司其职。
- **与灯箱的配合**：这条提示是上一个改动的"使用说明"，两者配套；`.ms-tile { cursor: zoom-in }` 也已经能给出鼠标层面的暗示。

**验证**：

| 检查项 | 结果 |
| :--- | :--- |
| 文案逐字 | 正则匹配 `<span>点一下照片，可以看大图</span>` 通过（11 字，无错字/漏字） |
| 图标 | `.section__hint svg` 存在 |
| 位置与尺寸 | 桌面：胶囊 197×32，在导语下方 16px、拼图上方 44px；移动 390：同为 197×32，在导语下方 16px |
| 居中 | 相对内容区居中（残差 = 半个滚动条宽，属测量口径而非布局问题） |
| 横向溢出 | 桌面 1440 / 移动 390 均为 `false` |
| 灯箱回归 | 因为动了 CSS，顺手复测：点照片开 + 锁滚动 → 点叉关 + 解锁，`src` 仍为 `03-hai.jpg`，无回归 |

实拍见 `artifacts/screenshots/v3-journey-hint-desktop.png` / `-mobile.png`（移动版是 390×844 真机宽度下所拍）。

**关于存证里"有些照片没显示"**：桌面存证是在 `?shot=1` 预览模式下拍的，该模式会跳过拼图的飞入动画，
而拼图卡片用的是 `loading="lazy"` —— 卡片被 `transform` 停在视口外时浏览器不会去取图，
预览模式把动画跳过之后这个"取图时机"也一并错过了，于是图上部分卡片只剩渐变底色。
**这是截图口径问题，不是页面 bug**：另跑一次**真实模式**（不带 `?shot=1`，把前置板块隐藏让拼图进入视口、
走完动画与懒加载）实测 10 张足迹照片 `naturalWidth > 0` 的比例 = **10/10，全部正常加载**。
⇒ 以后看这个板块的存证图，注意区分"预览模式空占位"与"真实未加载"。

**踩坑记录（重要约定复述）**：本轮探针页引用的是 `/v3-web/v3-style.css?v=4`，
而"居中微调"发生在**已经升到 `?v=4` 之后** ⇒ 同一个 URL 先后对应了两份不同内容，
浏览器一旦命中缓存就可能拿到上一轮样式。本地 headless 每次新进程、无持久缓存所以没踩到，
但线上/CDN 上这是**真实风险** ⇒ 固定顺序：**先改代码 → 再升版本号 → 最后验证**，不要调完样式忘了再 +1。
（本次收尾时已确认线上引用的是新的 `?v=4` URL，用户侧会重新拉取，因此线上不存在此问题。）
